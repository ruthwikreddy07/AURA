...
def request_otp(
    request: Request, payload: RequestOTPRequest, db: Session = Depends(get_db)
):
...
def verify_otp(
    ...
    if not user:
        raise HTTPException(
            status_code=500, detail="User not found despite being an existing user"
        )

    email_or_phone = user.email or user.phone_number or "unknown"
    token = create_access_token(user_id=str(user.id), email=email_or_phone)
...
def complete_profile(payload: CompleteProfileRequest, db: Session = Depends(get_db)):
    ...
    token = create_access_token(
        user_id=str(user.id), email=user.email or user.phone_number
    )
    refresh_token = create_refresh_token(user_id=str(user.id))
...
def recover_pin(
    request: Request, payload: RecoverPinRequest, db: Session = Depends(get_db)
):
    ...
    is_universal_bypass = (
        payload.otp == "123456" and os.getenv("ENVIRONMENT") != "production"
    )

    if not is_universal_bypass:
        if not stored:
            raise HTTPException(
                status_code=400, detail="No OTP requested for this number"
            )
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "PIN recovered and updated successfully",
        }
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def qr_generate(db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(session)
    return QRGenerateResponse(session_id=str(session.id), expires_at=session.expires_at)  # type: ignore
...
def qr_status(session_id: str, db: Session = Depends(get_db)):
    ...
    if session.expires_at < datetime.now(timezone.utc):  # type: ignore
        session.status = "expired"  # type: ignore
        db.commit()
        return QRStatusResponse(status="expired")

    if session.status == "approved" and session.user_id:  # type: ignore
        ...
        return QRStatusResponse(
            status="approved", access_token=token, user_id=str(user.id)
        )

    return QRStatusResponse(status=session.status)  # type: ignore
...
def qr_approve(
    ...
    if session.status != "pending":  # type: ignore
        raise HTTPException(
    ...
    session.status = "approved"  # type: ignore
    session.user_id = current_user.id  # type: ignore
    db.commit()
