...
def test_get_transactions_unauthorized(client):
    response = client.get("/api/v1/transactions/user/some-user-id")
    assert response.status_code == 401


def test_get_transactions_authorized_returns_list(auth_client):
    response = auth_client.get(f"/api/v1/transactions/user/{TEST_USER_ID}")
...
def test_create_transaction_unauthorized(client):
    response = client.post("/api/v1/transactions/create", json={
        "sender_id": "abc",
        "receiver_id": "def",
        "token_id": "ghi",
        "mode": "QR",
...
def test_create_transaction_invalid_token(auth_client):
    ...
    import uuid
    response = auth_client.post("/api/v1/transactions/create", json={
...
def test_get_transactions_with_filters(auth_client):
    ...
    response = auth_client.get(
        f"/api/v1/transactions/user/{TEST_USER_ID}",
